 ...
 *-------------------------------------------------------------------------
 *
 * $Source: /cvs/hopencvs/src/include/hopen/usb.h,v $
 * $Name:  $
 *
 * $Revision: 1.2 $     $Date: 2004/03/31 02:33:44 $
 *
\**************************************************************************/


#ifndef __HOPEN_USB_H
#define __HOPEN_USB_H
...
{
#if 0
	if( !_KERN_InInterrupt && ms >= 10 ) {
		int timeout = mini_to_ticks(ms);
		start_poll();
		poll_sleep_timeout(NULL, timeout);
		end_poll();
	} else
#endif
		mdelay(ms);
...
struct usb_endpoint_descriptor {
	...
	unsigned char  bRefresh		__attribute__ ((packed));
	unsigned char  bSynchAddress	__attribute__ ((packed));
...
struct usb_device_id {
	...
	unsigned short		idVendor;
	unsigned short		idProduct;
	unsigned short		bcdDevice_lo, bcdDevice_hi;
...
extern int usb_control_msg(struct usb_device *dev, unsigned int pipe,
				unsigned int request, unsigned int requesttype,
				unsigned int value, unsigned int index, void *data,
				unsigned int size, int timeout);

extern int usb_root_hub_string(int id, int serial, char *type, unsigned char *data, int len);
...
#define __create_pipe(dev, endpoint) (((dev)->devnum << 8) | ((endpoint) << 15) | ((dev)->slow << 26))
#define __default_pipe(dev)	((dev)->slow << 26)
...
int usb_get_descriptor(struct usb_device *dev, unsigned int desctype,
	unsigned int descindex, void *buf, int size);
