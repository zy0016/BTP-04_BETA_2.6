#ifndef _MTD_CFI_CMDSET_0001_H
#define _MTD_CFI_CMDSET_0001_H
...
#define PARTITION_SEL(chip, adr) ({	\
	...
	struct flhwpart *__p=NULL, *__p0;	\
	struct flprivate *__priv = (struct flprivate*) chip->priv; \
	for (__num=0; __num < __priv->num_hwparts; __num++) {	\
		__p0 = __priv->hwparts + __priv->curpos; \
		__start = __p0->offset; \
		...
		__priv->curpos = (__priv->curpos + 1) % __priv->num_hwparts; \
	} \
	__p; \
})
...
#define CFI_QUERY_SIZE(map, base, adr) ({ \
	...
	if (__ext.MajorVersion == '1' && __ext.MinorVersion == '3') { \
		__ofs += (__ext.NumProtectionFields - 1) * (sizeof(unsigned long) + 6); \
		__ofs += 6; \
