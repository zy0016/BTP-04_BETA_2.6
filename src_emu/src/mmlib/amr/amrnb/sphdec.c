 ...
 * Module   : sphdec.c
 *
 * Purpose  : Define functions of speech decoder.
 *
\**************************************************************************/
                     ...
                     Word16 *indice, Word32 *lsp1_q)
{
    ...
    if (bfi != 0)
    {
        ...
        for (i = 0; i < M; i ++)
        {
            ...
            lsf1_q[i] = ((st->past_lsf_q[i] * ALPHA) >> 15)
                + ((mean_lsf_3[i] * ONE_ALPHA) >> 15);
        }
    ...
    else
    {
        ...
        if (mode != MRDTX)
        {
            for (i = 0; i < M; i ++)
            {
                lsf1_q[i] = lsf1_r[i] + (mean_lsf_3[i]
...
static void Lsp_lsf(Word32 lsp[], Word32 lsf[])
{
    ...
    for (i = M - 1; i >= 0; i --)
    {
        ...
        lsf[i] = ((((lsp[i] - cos_table[ind]) * acos_slope[ind])
            + 0x800) >> 12) + (ind << 8);
    }
...
static Word32 Syn_filt(Word32 a[], Word32 x[], Word32 y[],
                       Word32 lg, Word32 mem[], Word32 update)
{
...
static void Syn_filt_overflow(Word32 a[], Word32 x[], Word32 y[],
                              Word32 lg, Word32 mem[], Word32 update)
{
    Word32 tmp[50];   /* malloc is slow */
    Word32 i, j, s, a0;
...
static void dtx_dec(dtx_decState *st, Word32 *mem_syn, D_plsfState *lsfState,
                    ...
                    enum DTXStateType new_state, enum Mode mode,
                    Word16 parm[], Word32 synth[], Word32 A_t[])
{
        ...
        for (i = 0; i < M; i ++)
        {
            ...
            */
            for (j = 0; j < 8; j ++)
            {
                ...
                {
                    st->lsf_hist_mean[i + j * M] = 655
                        + ((st->lsf_hist_mean[i + j	* M] - 655) >> 2);
                }
    ...
    * even if there is no new valid_data
        ...
        * interpolation to 32 frames
            */
            ...
            if (tmp_int_length >= 2)
            {
                st->true_sid_period_inv = 0x2000000
                    / (tmp_int_length << 10);
            }
            else
            {
    ...
    */
    ...
    for ( i = 0; i < M; i++ )
    {
        lsf_int_variab[i] = lsf_int_variab[i] + ((lsf_variab_factor
...
static void Int_lpc_1and3(Word32 lsp_old[], Word32 lsp_mid[],
                          Word32 lsp_new[], Word32 Az[])
{
    Word32 lsp[M];
    Word32 i;
...
static void Int_lpc_1to3(Word32 lsp_old[], Word32 lsp_new[],
                         Word32 Az[])
{
    Word32 lsp[M];
    Word32 i;
                     ...
                     Word32 *lsp1_q, Word32 *lsp2_q)
{
    ...
    if (bfi != 0)
    {
        ...
        for (i = 0; i < M; i += 2)
        {
            ...
            lsf1_q[i] = ((st->past_lsf_q[i] * ALPHA_122) >> 15 )
                + ((mean_lsf_5[i] * ONE_ALPHA_122) >> 15);
            lsf1_q[i + 1] = ((st->past_lsf_q[i + 1] * ALPHA_122)
                >> 15) + ((mean_lsf_5[i + 1] * ONE_ALPHA_122) >> 15);
        }
        ...
        for (i = 0; i < M; i += 2)
        {
            ...
            temp1 = mean_lsf_5[i] + ((st->past_r_q[i]
                * LSP_PRED_FAC_MR122) >> 15);
            temp2 = mean_lsf_5[i + 1] + ((st->past_r_q[i + 1]
                * LSP_PRED_FAC_MR122) >> 15);
...
static void Dec_lag3(Word32 index, Word32 t0_min, Word32 t0_max,
                     Word32 i_subfr, Word32 T0_prev, Word32 *T0,
...
static void Pred_lt_3or6_40(Word32 exc[], Word32 T0,
                            Word32 frac, Word32 flag3)
{
    Word32 s, i;
    Word32 *x0, *x1, *x2;
    const Word32 *c1, *c2;
...
static void Dec_lag6(Word32 index, Word32 pit_min, Word32 pit_max,
                     Word32 i_subfr, Word32 *T0, Word32 *T0_frac)
{
...
static void decompress10(Word32 MSBs, Word32 LSBs, Word32 index1,
                         Word32 index2, Word32 index3, Word32 pos_indx[])
{
    Word32 divMSB;
    ...
    */
    divMSB = MSBs / 25;
    pos_indx[index1] = (((MSBs - 25 * (divMSB)) % 5) << 1)
        + (LSBs & 0x1);
    pos_indx[index2] = (((MSBs - 25 * (divMSB)) / 5) << 1 )
        + ((LSBs & 0x2) >> 1);
...
static void decode_3i40_14bits(Word32 sign, Word32 index,
                               Word32 cod[])
{
    Word32 pos[3];
    Word32 i, j;
...
static void decode_4i40_17bits(Word32 sign, Word32 index,
                               Word32 cod[])
{
    Word32 pos[4];
    Word32 i, j;
...
static void gc_pred(gc_predState *st, enum Mode mode, Word32 *code,
                    Word32 *exp_gcode0, Word32 *frac_gcode0,
...
static void Dec_gain(gc_predState *pred_state, enum Mode mode,
                     Word32 index, Word32 code[], Word32 evenSubfr,
...
static void gc_pred_average_limited(gc_predState *st,
                                    Word32 *ener_avg_MR122, Word32 *ener_avg)
{
    Word32 av_pred_en, i;
...
static void ec_gain_code(ec_gain_codeState *st,
                         gc_predState *pred_state, Word16 state, Word32 *gain_code)
{
...
static void d_gain_code(gc_predState *pred_state, enum Mode mode,
                        Word32 index, Word32 code[], Word32 *gain_code)
{
...
static Word32 Cb_gain_average(Cb_gain_averageState *st,
                              enum Mode mode, Word32 gain_code, Word32 lsp[],
                              ...
                              Word32 inBackgroundNoise, Word32 voicedHangover)
{
    ...
    */
    ...
    diff = *tmp + tmp[1] + tmp[2] + tmp[3] + tmp[4] + tmp[5]
    ...
    if ((mode <= MR67) | (mode == MR102))
    {
        ...
        */
        sum = st->cbGainHistory[2] + st->cbGainHistory[3]
            + st->cbGainHistory[4] + st->cbGainHistory[5]
            + st->cbGainHistory[6];
        ...
        if (((bfi != 0) | (prev_bf != 0)) & (inBackgroundNoise != 0 )
            & (mode < MR67))
        {
            sum = 9362 * (st->cbGainHistory[0] + st->cbGainHistory[1]
                + st->cbGainHistory[2] + st->cbGainHistory[3]
                + st->cbGainHistory[4] + st->cbGainHistory[5]
                + st->cbGainHistory[6]);
                    ...
                    Word32 pitch_fac, Word32 tmp_shift)
{
    ...
    */
    if ((mode != MR122) & (mode != MR102) & (mode != MR74)
        & (impNr < 2 ))
    {
    /*
...
static Word16 Ex_ctrl(Word32 excitation[], Word32 excEnergy,
                      Word32 exEnergyHist[], Word32 voicedHangover,
                      Word16 prevBFI,	Word16 carefulFlag)
{
...
static Word16 Bgn_scd(Bgn_scdState *st, Word32 ltpGainHist[],
                      Word32 speech[], Word32 *voicedHangover)
{
...
static void Decoder_amr(Decoder_amrState *st, enum Mode mode,
                        Word16 parm[], enum RXFrameType frame_type,
    ...
    */
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
        ...
        * this is needed for mode MR475, MR515, MR59 and MR67
        ...
        {
            Dec_lag6(index, PIT_MIN_MR122, PIT_MAX,
                pit_flag, &T0, &T0_frac);
        ...
        */
        ...
        else if ((mode <= MR74) || (mode == MR102))
        {
            ...
            if (bfi == 0)
            {
                Dec_gain(st->pred_state, mode, index, code, evenSubfr,
                    &gain_pit, &gain_code);
            }
            else
            {
        ...
        */
        ...
        gain_code_mix = Cb_gain_average(st->Cb_gain_averState, mode,
            gain_code, lsf_i, st->lsp_avg_st->lsp_meanSave, bfi,
            st->prev_bf, pdfi, st->prev_pdf, st->inBackgroundNoise,
            st->voicedHangover);
        ...
        */
        ...
        if (((mode == MR475) || (mode == MR515) || (mode == MR59))
            ...
            & (st->state < 4 ) & (((pdfi != 0) & (st->prev_pdf != 0))
            || bfi != 0 || st->prev_bf != 0))
        {
            carefulFlag = 0;
        ...
        if ((st->inBackgroundNoise != 0) & (bfi != 0
            || st->prev_bf != 0) & (st->state < 4))
        {
            ;   /* do nothing! */
        }
        else
        ...
        */
        if (pit_sharp > 16384)
        {
            ...
            overflow = Syn_filt(Az, excp, &synth[i_subfr],
                L_SUBFR, st->mem_syn, 0);
        }
        else
        {
            overflow = Syn_filt(Az, exc_enhanced, &synth[i_subfr],
                L_SUBFR, st->mem_syn, 0);
        }
...
static void agc(agcState *st, Word32 *sig_in,
                Word32 *sig_out, Word16 agc_fac)
{
...
static void Post_Filter(Post_FilterState *st, enum Mode mode,
                        Word32 *syn, Word32 *Az_4)
{
    ...
    */
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
        ...
        *p1 = *p1 - ( ( temp2 * st->preemph_state_mem_pre ) >> 15 );
        ...
        overflow = Syn_filt(Ap4, st->res2, &syn[i_subfr], L_SUBFR,
            st->mem_syn_pst, 0);
        if (overflow)
        {
...
static void Post_Process(Post_ProcessState *st, Word32 signal[])
{
    ...
    do
    {
        ...
        *                    + a[1]*y[i-1] + a[2] * y[i-2];
        */
        tmp = (st->y1_hi * 15836) + (((st->y1_lo * 15836)
            & (Word32)0xffff8000) >> 15);
        tmp += (st->y2_hi * -7667) + (((st->y2_lo * (-7667))
            & (Word32)0xffff8000) >> 15);
        tmp += st->x0 * 7699;
        tmp += st->x1 * -15398;
        if (((tmp >> 1) ^ tmp) & mask)
...
void Speech_Decode_Frame(void *st, enum Mode mode, Word16 *parm,
                         enum RXFrameType frame_type, Word16 *synth)
{
    ...
    Decoder_amr(((Speech_Decode_FrameState *)st)->decoder_amrState,
        mode, parm, frame_type, synth_speech, Az_dec);
...
static int Decoder_amr_init(Decoder_amrState **state)
{
    ...
    *state = NULL;
    ...
    if ((s->lsfState = (D_plsfState *)malloc(sizeof(D_plsfState)))
        == NULL)
    {
        printf( "DPlsf_init: can not malloc state structure\n" );
        return-1;
    }
    ...
    {
        printf( "ecGainPitchInit: can not malloc state structure\n" );
        return-1;
    }
    ...
    {
        printf( "ecGainCodeInit: can not malloc state structure\n" );
        return -1;
    }
    ...
    if ((s->pred_state = (gc_predState *)malloc(sizeof(gc_predState)))
        == NULL)
    {
        printf( "gcPredInit: can not malloc state structure\n" );
        return -1;
    }
    ...
    {
        printf( "Cb_gain_averageInit: can not malloc state structure\n" );
        return -1;
    }
    ...
    if ((s->lsp_avg_st = (lsp_avgState *)malloc(sizeof(lsp_avgState)))
        == NULL)
    {
        printf( "lsp_avgInit: can not malloc state structure\n" );
        return -1;
    }
    
    /* Bgn_scdInit */
    /* allocate memory */
    if ((s->background_state = (Bgn_scdState *)malloc(sizeof(
        Bgn_scdState))) == NULL)
    {
        printf( "Bgn_scdInit: can not malloc state structure\n" );
        return -1;
    }
    ...
    if ((s->ph_disp_st = (ph_dispState *)malloc(sizeof(ph_dispState)))
        == NULL)
    {
        printf( "phDispInit: can not malloc state structure\n" );
        return -1;
    }
    
    /* dtxDecInit */
    /* allocate memory */
    if ((s->dtxDecoderState = (dtx_decState *)malloc(sizeof(
        dtx_decState))) == NULL)
    {
        printf( "dtxDecInit: can not malloc state structure\n" );
        return -1;
    }
...
static int Post_Filter_init(Post_FilterState **state)
{
    ...
    *state = NULL;
    ...
    if ((s = (Post_FilterState *)malloc(sizeof(Post_FilterState)))
        == NULL)
    {
        printf( "F057:can not malloc filter structure\n" );
        return-1;
    }
    s->agc_state = NULL;
    ...
    {
        printf( "agcInit: can not malloc state structure\n" );
        return -1;
    }
...
void Speech_Decode_Frame_exit(void **st)
{
    if (((Speech_Decode_FrameState *)(st)) == NULL)
        return;
    Decoder_amr_exit(&(((Speech_Decode_FrameState *)st)
        ->decoder_amrState));
    Post_Filter_exit(&(((Speech_Decode_FrameState *)st)
        ->post_state));
    Post_Process_exit(&(((Speech_Decode_FrameState *)st)
        ->postHP_state));
...
/*********************************************************************
* Function	   void *Speech_Decode_Frame_init()
* Purpose      Allocates state memory and initializes state memory.
* Params	   void
* Return	   Returns pointer to structure.
* Remarks	   
**********************************************************************/
