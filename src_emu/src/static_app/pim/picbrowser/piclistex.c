...
#define X_RIGHTICON       75
#define X_LEFTICON2       40
#define	CX_LEFTICON2	  27
#define LINE_PRESCROLL    1

#define MAX_ICON_NUM      3
...
LRESULT CALLBACK PICLISTExWndProc(HWND hWnd, UINT wMsgCmd, 
...
static LRESULT ProcessPen(HWND hWnd, PLISTBOXDATA pListBoxData, UINT wMsgCmd, 
                          WPARAM wParam, LPARAM lParam);
static void Bitmap_Paint(HDC hdc, HANDLE hImage, RECT* pRect,DWORD fImageType);
...
BOOL PICLISTEX_RegisterClass(void)
{
    ...
    wc.lpfnWndProc      = PICLISTExWndProc;
    wc.cbClsExtra       = 0;
...
LRESULT CALLBACK PICLISTExWndProc(HWND hWnd, UINT wMsgCmd, WPARAM wParam, LPARAM lParam)
{
    LRESULT         lResult;
    ...
    case WM_CREATE :
        ...
        {
            nWidth -= 10;
            pListBoxData->byPageTextLen = (BYTE)(nWidth / size.cx);
            //if (nWidth % size.cx)
        ...
        if (lpsz)
        {
            ...
            else
            {
                ...
                if (oldLen < len)
                {
                    ...
                    if (p->hImage[0])
					{
						nFileType = p->fImageType;
                        hImage = p->hImage[0];
					}
                    ItemData = p->dwNodeData;
					if (p->pszAuxText)
					{
						pAuxtext = malloc(strlen(p->pszAuxText)+1);
						if (pAuxtext)
						{
							strcpy(pAuxtext, p->pszAuxText);	
						}
					...
					{
                        s->hImage[0] = hImage;
						s->fImageType = nFileType;
					}
                    s->dwNodeData = ItemData;
					if (pAuxtext)
					{
						s->pszAuxText = malloc(strlen(pAuxtext) + 1);
						if (s->pszAuxText)
						{
							strcpy(s->pszAuxText, pAuxtext);
							s->wAuxTextLen = strlen(s->pszAuxText);
						}
						if (pAuxtext)
						{
							free(pAuxtext);
    ...
    case LB_GETIMAGE :
		{
			...
			index = LOBYTE(HIWORD(wParam));      // index
			nIconPos = HIBYTE(HIWORD(wParam));   // icon position
...
static void LISTBOX_Paint(HWND hWnd, HDC hdc, PLISTBOXDATA pListBoxData, 
                          ...
                          int nColStart, int nColEnd)
{
    RECT        rcList, rect, rect1, rcLine1, rcLine2, rcLine1Text;
    PLISTNODE   p;
    DWORD       dwStyle;
	WORD	    j, wPageCount;
    SIZE        size;
...
//            crBkOld = SetBkColor(hdc, RGB(193, 213, 237));
        ...
        {
            RECT    rcImage, rcImage1, rcImage2;
            IWORD   wStrLen;
            ...
            */
			SetRect(&rcImage, rect1.left +  CX_LEFTMARGIN,
				rect1.top + (nItemHeight - pListBoxData->wBitmapHeight[0])/2,
				rect1.left + pListBoxData->wBitmapWidth[0] + CX_LEFTMARGIN,
				rect1.top + (nItemHeight + pListBoxData->wBitmapHeight[0])/2);
            if (p->hImage[0])
                Bitmap_Paint(hdc, p->hImage[0], &rcImage, p->fImageType);
...
//            }
            ...
            if (p->hImage[1] != NULL)
            {
                if (dwStyle & LBS_MULTILINE)
                {
                    SetRect(&rcImage1, rect1.left + CX_LEFTMARGIN + X_RIGHTICON,
                        rect1.top + CY_TOPMARGIN,
                        rect1.left + CX_LEFTMARGIN + X_RIGHTICON + CX_RIGHTICON,
                        rect1.bottom);
                }
                else
                {
                ...
                Bitmap_Paint(hdc, p->hImage[1], &rcImage1, p->fImageType);
            }

            if (p->hImage[2] != NULL)
            {
				SetRect(&rcImage2, rcImage.right +  CX_LEFTMARGIN,
				rect1.top + (nItemHeight - pListBoxData->wBitmapHeight[2])/2,
				rcImage.right + pListBoxData->wBitmapWidth[2] + CX_LEFTMARGIN,
				rect1.top + (nItemHeight + pListBoxData->wBitmapHeight[2])/2);

				/*
                if (dwStyle & LBS_MULTILINE)
                {
                    SetRect(&rcImage2, rect1.left + CX_LEFTMARGIN + CX_LEFTICON ,
                        rect1.top + CY_TOPMARGIN,
                        rect1.left + CX_LEFTMARGIN + CX_LEFTICON  + CX_LEFTICON2,
                        rect1.bottom);
                }
                else
                {
                    SetRect(&rcImage2, rect1.right - CX_RIGHTICON - 2 * CX_RIGHTMARGIN,
                        rect1.top, rect1.right - CX_RIGHTMARGIN, rect1.bottom);
                }
				*/
                Bitmap_Paint(hdc, p->hImage[2], &rcImage2, p->fImageType);
            }
...
//            else
                ...
                || (p->wAuxTextLen > pListBoxData->wStartTextPos))
            {
                ...
                {
                    //nIndent += CX_LEFTICON;
					nIndent += pListBoxData->wBitmapWidth[0];
                }
				if (pListBoxData->wBitmapWidth[2] != 0)
                {
                    //nIndent += CX_LEFTICON2;
					nIndent += pListBoxData->wBitmapWidth[2] + CX_LEFTMARGIN;
                }

                if (!(dwStyle & LBS_MULTILINE))
                {
                    nIndent -= 3;
                    nTextWidth = rect1.right - (rect1.left + nIndent) -2;
                    ...
                    if((numstr = strstr(p->pNodeString,SEPARATORS))==NULL)
					{
						GetTextExtentPoint32(hdc, p->pNodeString, p->wStrLen, &size);
						GetExtentFittedText(hdc, p->pNodeString, p->wStrLen,
							szTextDisplay, MAX_DISPLAYTEXT, nTextWidth - CX_LEFTMARGIN, '.', 3);
						
						//DrawText(hdc, szTextDisplay, -1, &rect1, DT_LEFT|DT_VCENTER);
						
						TextOut(hdc, rect1.left + nIndent + CX_LEFTMARGIN,
					...
					{						
						...
						GetFontHandle(&hSmall,0);
						//		token = pNode->pNodeString;
						//		*(token+(numstr-2-token)) ='\0';
						strncpy(token,p->pNodeString,numstr - p->pNodeString);
						numstr+=2;
						SetRect(&rcLine1Text, rect1.left+nIndent + CX_LEFTMARGIN, rect1.top ,rect1.right-7, rect1.bottom);

						hOldFont = SelectObject(hdc, hSmall);
						GetTextExtentPoint32(hdc, numstr, strlen(numstr), &size);
						
						SelectObject(hdc, hOldFont);
						nTextWidth = nTextWidth - size.cy;
						GetExtentFittedText(hdc, token, strlen(token),
							szTextDisplay, MAX_DISPLAYTEXT, nTextWidth - CX_LEFTMARGIN, '.', 3);

						
						DrawText(hdc, szTextDisplay, strlen(szTextDisplay), &rcLine1Text, DT_LEFT|DT_VCENTER);
						
						
						//DrawText(hdc, token, strlen(token), &rcLine1Text, DT_LEFT|DT_VCENTER);
						
						
						
						hOldFont = SelectObject(hdc, hSmall);
						DrawText(hdc, numstr, strlen(numstr), &rcLine1Text, DT_RIGHT|DT_VCENTER);
...
//					DrawText(hdc, szTextDisplay, -1, &rect1, DT_LEFT|DT_VCENTER);
                ...
                else
                {
					...
					if((numstr = strstr(p->pNodeString,SEPARATORS))==NULL)
					{
						...
						TextOut(hdc, rcLine1.left + nIndent + CX_LEFTMARGIN,
					...
					else
					{
						...
						strncpy(token,p->pNodeString,numstr - p->pNodeString);
						numstr+=2;
						SetRect(&rcLine1Text, rect1.left+nIndent + CX_LEFTMARGIN, rect1.top, rect1.right -CX_LEFTMARGIN, rcLine1.bottom);
					...
					*/
                    ...
                    if (p->pszAuxText != NULL)
                    {
                        ...
                        //GetExtentFittedText(hdc, p->pszAuxText, p->wAuxTextLen,
                            //szTextDisplay, MAX_DISPLAYTEXT, nTextWidth - CX_LEFTMARGIN, '.', 3);
                        TextOut(hdc, rcLine2.left + nIndent + CX_LEFTMARGIN,
...
static void Bitmap_Paint(HDC hdc, HANDLE hImage, RECT* pRect, DWORD filetype)
{
    char PathName[256];
...
char * PicLocalStringEx( int nSize)
{
    char* pTemp; 
...
//    if (!IsWindowVisible(hWnd)) //For comboListbox.
                         ...
                         int *pnLine1Height, int *pnLine2Height)
{
    ...
    else
    {
		...
		nLine1Height = sizeLine1.cy + SPACE;
		...
		nLine2Height = sizeLine2.cy + SPACE - 1;
