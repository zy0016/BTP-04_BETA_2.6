...
static LRESULT CALLBACK PicMultiListExProc(HWND hWnd, UINT uMsgCmd, 
...
BOOL PICMULTILISTEX_RegisterClass(void)
{
    ...
    GetClassInfo(NULL, "PICLISTEX", &wc);
	
	wc.style            = CS_OWNDC;
    wc.lpfnWndProc      = PicMultiListExProc;
    wc.cbClsExtra       = 0;
    ...
    wc.lpszClassName    = "PICMULTILISTEX";
                                 ...
                                 WPARAM wParam, LPARAM lParam)
{
	...
	GetClassInfo(NULL, "PICLISTEX", &WndClassList);
    ...
    if ((uMsgCmd == WM_KEYDOWN) && (wParam == VK_DOWN || wParam == VK_UP)
        ...
        if (nIndexLast != LB_ERR)
            bSelLast = CallWindowProc(PicMultiListExProc, hWnd, LB_GETSEL, nIndexLast, 0);
    }
	...
	case WM_KEYDOWN:
        ...
        case VK_UP:
            ...
            bSelected = CallWindowProc(PicMultiListExProc, hWnd, LB_GETSEL,
                nCurIndex, 0);
            if (nIndexLast != LB_ERR)
            {
            CallWindowProc(WndProcList, hWnd, LB_SETIMAGE, 
