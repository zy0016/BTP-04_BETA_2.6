...
static char * pClassName = "SetGPSOptiClass";
...
BOOL CallSetGPSOpti(HWND hParentWnd, int iGPSIndex)
{
    WNDCLASS    wc;
    RECT rClient;
    ...
    SetWindowText(hFrameWin, ML("Optimized for"));
    SetFocus(hWndApp);
...
static BOOL CreateControl (HWND hWnd, HWND *hList)
{
    ...
    * hList = CreateWindow("LISTBOX", 0, 
    ...
    SendMessage(* hList, LB_ADDSTRING, 0, (LPARAM)ML("Accuracy"));
    SendMessage(* hList, LB_ADDSTRING, 0, (LPARAM)ML("Sensitivity"));
