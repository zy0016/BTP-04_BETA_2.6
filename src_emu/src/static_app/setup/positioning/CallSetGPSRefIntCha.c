...
static char * pClassName = "SetGPSRefIntChaClass";
...
BOOL CallSetGPSRefIntCha(HWND hParentWnd, int iGPSIndex)
{
    WNDCLASS    wc;
    RECT rClient;
    ...
    SetWindowText(hFrameWin, ML("Ref. int. charging"));
    SetFocus(hWndApp);
...
static BOOL CreateControl (HWND hWnd, HWND *hList)
{
    ...
    * hList = CreateWindow("LISTBOX", 0, 
    ...
    SendMessage(* hList, LB_ADDSTRING, 0, (LPARAM)ML("Continuous"));
    SendMessage(* hList, LB_ADDSTRING, 0, (LPARAM)ML("Keep interval"));
