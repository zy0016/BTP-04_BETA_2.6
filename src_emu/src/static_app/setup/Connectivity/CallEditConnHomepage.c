...
#include    "mullang.h"
#include	"imesys.h"
#include    "CallEditConnHomepage.h"

static HWND hAppWnd;
static HWND hFrameWin;
static int iCurMode;
static UDB_ISPINFO Global_IspInfo;
static char szConnHomepage[UDB_HOMEPAGE_SIZE +1] = "";
static const char* pClassName = "EditConnHomepageClass";
...
BOOL CallEditConnHomepage(HWND hWndApp,int icurMode)
{   
    WNDCLASS wc;
    RECT rClient;
...
static LRESULT  AppWndProc( HWND hWnd, UINT wMsgCmd, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_COMMAND:            
        ...
        case IDC_BUTTON_SAVE: 
            GetWindowText(hwndEdit,szConnHomepage,sizeof(szConnHomepage));
			
			Sett_SpecialStrProc(szConnHomepage);

//			if ( 0 == strlen(szConnHomepage) )
//			{
//				PLXTipsWin(NULL, NULL, NULL,ML("Please define homepage"), NULL, Notify_Alert, ML("Ok"), NULL, WAITTIMEOUT);
//				break;
//			}
            strncpy(Global_IspInfo.ConnectHomepage,szConnHomepage,sizeof(szConnHomepage));
...
static BOOL		CreateControl (HWND hWnd, HWND *hwndEdit)
{
   ...
   * hwndEdit = CreateWindow(
        "IMEEDIT",
        Global_IspInfo.ConnectHomepage,
   ...
   SendMessage(* hwndEdit, EM_LIMITTEXT, UDB_HOMEPAGE_SIZE, 0);
   
   SendMessage(* hwndEdit, EM_SETTITLE, 0, (LPARAM)ML("Homepagecolon"));
