...
#include    "mullang.h"
#include	"imesys.h"
#include    "CallEditConnPort.h"

static HWND hAppWnd;
static HWND hFrameWin;
static int iCurMode;
static UDB_ISPINFO Global_IspInfo;
static char szConnPort[UDB_K_PORT_SIZE +1] = "";
static const char* pClassName = "EditConnectionPortClass";
...
BOOL CallEditConnPort(HWND hWndApp,int icurMode)
{   
    WNDCLASS wc;
    RECT rClient;
...
static LRESULT  AppWndProc( HWND hWnd, UINT wMsgCmd, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_COMMAND:            
        ...
        case IDC_BUTTON_SAVE: 
            GetWindowText(hwndEdit,szConnPort,sizeof(szConnPort));
            strncpy(Global_IspInfo.ConnPort,szConnPort,sizeof(szConnPort));
...
static BOOL		CreateControl (HWND hWnd, HWND *hwndEdit)
{
   ...
   * hwndEdit = CreateWindow(
   ...
   SendMessage(* hwndEdit, EM_LIMITTEXT, UDB_K_PORT_SIZE, 0);
   
   SendMessage(* hwndEdit, EM_SETTITLE, 0, (LPARAM)ML("Portcolon"));
