...
#include    "mullang.h"
#include	"imesys.h"
#include    "CallEditGPRSAccPoint.h"

static HWND hAppWnd;
static HWND hFrameWin;
static int iCurMode;
static UDB_ISPINFO Global_IspInfo;
static char cCurMode[UDB_K_TEL_SIZE +1] = "";
static const char* pClassName = "EditGPRSAccessPointClass";
...
BOOL CallEditGPRSAccPoint(HWND hWndApp,int icurMode)
{   
    WNDCLASS wc;
    RECT rClient;
...
//				break;				
//			}
            strncpy(Global_IspInfo.ISPPhoneNum1,cCurMode,sizeof(cCurMode));
...
static BOOL		CreateControl (HWND hWnd, HWND *hwndEdit)
{
   ...
   * hwndEdit = CreateWindow(
   ...
   SendMessage(* hwndEdit, EM_SETTITLE, 0, (LPARAM)ML("Access point namecolon"));
