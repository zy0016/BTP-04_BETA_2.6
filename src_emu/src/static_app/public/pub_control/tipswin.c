...
--------------------------------------------------------------------------
  IMPORTANT NOTES:
              None.
*************************************************************************/
void PLXTipsWin(HWND hFrame, HWND hParent, UINT	rMsg, PCSTR szPrompt,PCSTR szCaption,NOTIFYTYPE Tipspic,PCSTR szOK, PCSTR szCancel, int nTimeout )
{
        ...
        case WM_PAINT:
			{
				...
				if(DrawImageFromFile(hdc, pMsgCurrentWin->strCongif, 80, 8, ROP_SRC))
					printf("succeded to drawimage %s", pMsgCurrentWin->strCongif);
				else
					printf("failed to drawimage %s", pMsgCurrentWin->strCongif);
						   ...
						   char *szPrompt, int nTimeout, NOTIFYTYPE eTypePic, UINT rMsg)
{
    ...
    if(szPrompt != NULL)
    {
		...
		memset(pMsgCurrentWin->sMsgWinPrompt, 0, strlen(szPrompt)+1);
		strcpy(pMsgCurrentWin->sMsgWinPrompt,szPrompt);
	...
	{
        pMsgCurrentWin->sMsgWinPrompt = malloc(2);
		pMsgCurrentWin->sMsgWinPrompt[0] = 0;
	}
...
BOOL	UpdateTipswinPrompt(PCSTR pBuf)
{
	...
	while (pMsgWin)
	{
		if (IsWindowVisible(pMsgWin->hMsgwin))
		{
			break;
			//return TRUE;
		}
	...
	if (pMsgWin)
	{
		pMsgWin->sMsgWinPrompt = realloc(pMsgWin->sMsgWinPrompt, strlen(pBuf)+1);
		strcpy(pMsgWin->sMsgWinPrompt, pBuf);
		InvalidateRect(pMsgWin->hMsgwin, NULL, TRUE);
	}
	else
	{
