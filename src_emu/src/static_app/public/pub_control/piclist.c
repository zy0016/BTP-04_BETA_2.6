...
LRESULT CALLBACK PICLIST_WndProc(HWND hWnd, UINT wMsgCmd, 
...
BOOL PICLIST_RegisterClass(void)
{
    ...
    wc.lpfnWndProc      = PICLIST_WndProc;
    wc.cbClsExtra       = 0;
...
LRESULT CALLBACK PICLIST_WndProc(HWND hWnd, UINT wMsgCmd, WPARAM wParam, LPARAM lParam)
{
    LRESULT         lResult;
        ...
        if (wParam > 0)
        {
			...
			if (!(dwStyle & LBS_USERSTRINGS))
			{
				...
				pListBoxData->pStringCache = pListBoxData->pCurString =
	                PicLocalString(pListBoxData->wStringCount);
			}
        }
