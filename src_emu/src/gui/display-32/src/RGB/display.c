...
static int OpenDevice(PDEV pDev, int dev)
{
	...
	//	printf("&&&&&&&&&&&&&&&&&&&&& Opendevice SCREEN_MAIN\r\n");
    	break;
#ifdef PIP_TRANS
	case SCREEN_BACKGROUND:
		pDisplayDriver = EnableRgbDisplayDev(NULL);
	//	printf("&&&&&&&&&&&&&&&&&&&&& Opendevice SCREEN_BACKGOURND\r\n");
		break;
#endif
    default:
        return -1;
...
void SetSignificant(BOOL bSet)
{
   bSetSign = bSet;
}

void Put32_DWEx(uint8 *p, uint32 pixel)
{   
    if (bSetSign)
        *((DWORD*)p) = ((DWORD)pixel | (1L << 18));
    else
        *((DWORD*)p) = (DWORD)pixel;
}

uint32 REALIZECOLOR32_0x0R6G6B6Ex(uint32 r, uint32 g, uint32 b)
{
    uint32 color;

    if (bSetSign)
        color = ((((r) << 10) & 0x3f000) | (((g) << 4) & 0x00fc0) | ((b) >> 2) | (1L << 18));
    else
        color = ((((r) << 10) & 0x3f000) | (((g) << 4) & 0x00fc0) | ((b) >> 2));
    return color;
}
