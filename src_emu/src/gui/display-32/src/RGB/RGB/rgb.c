...
#include "dispfunc.h"
#ifdef PIP_EMULATE
static uint8 *pDataBack = NULL, *pDataSrc = NULL;
#endif
...
#ifdef PIP_EMULATE
        if (dev == SCREEN_BACKGROUND)
        {
            if (!PDD_GetDisplayMode(0, &DspMode))
                return -1;
            if (!pDataBack)
            {
                pDataBack = (uint8*)malloc(DspMode.dsp_scanline_bytes * DspMode.dsp_height);
                memset(pDataBack, 0, DspMode.dsp_scanline_bytes * DspMode.dsp_height);
                pDataSrc = (uint8*)malloc(DspMode.dsp_scanline_bytes * DspMode.dsp_height);
                memset(pDataSrc, 0, DspMode.dsp_scanline_bytes * DspMode.dsp_height);
            }
        }
        else
#endif
...
#ifdef PIP_EMULATE
        if (dev == SCREEN_BACKGROUND)
            pDev->pdata = pDataBack;
        else if (dev == SCREEN_MAIN)
            pDev->pdata = pDataSrc;
        else
#endif
