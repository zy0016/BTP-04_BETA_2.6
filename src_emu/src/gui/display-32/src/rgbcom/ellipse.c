...
//        pencolor = pGraphParam->pPen->fgcolor;
    ...
    flagMiddlePoint = (int32)(Ry2 * (2 * x + 1) * (2 * x + 1) / 4 + 
        Rx2 * (y - 1) * (y - 1) - Rx2 * Ry2);
    ...
    if (flagYEven == 0)
    {
        ...
        else if (IsPtInYClip(rcClip, 0))
        {
            ...
            /* WWH modified on 2004/09/09 
            ** The following should be in the Y clip
            */
            if (y != yLast)
...
//    pencolor = pGraphParam->pPen->fgcolor;
    ...
    //绘制第二部分
    yLoopEnd = (flagYEven == 1) ? 0 : 1;
    flagMiddlePointOut = (int32)(Ry2Out * (2 * xOut + 1) * (2 * xOut + 1) / 4 + 
    ...
    while (yOut >= yLoopEnd) //切线斜率< -1, Y控制
    {
        ...
        else
        {
            ...
            if (flagInSecond == 0)
            {
                ...
                {
                    flagMiddlePointIn = (int32)(Ry2In * (2 * xIn + 1) * (2 * xIn + 1) / 4 + 
                        ...
                        PROUNDRECT pRoundRectData)
{
    ...
    /* WWH modified on 2004/04/23
    ** The input width and height should be the value of the total ellipse.
    ** So here should be devided by 2.
    */
    width = pRoundRectData->width / 2;
    height = pRoundRectData->height / 2;
    if ((pRoundRectData->x2 - pRoundRectData->x1) <= (width * 2))
    {
        if ((pRoundRectData->y2 - pRoundRectData->y1) <= (height * 2))
        {
            ...
            //DrawThinEllipse haven't the error judgement
//            return DrawThinEllipse(pDev, pGraphParam, &ellipsedata);
                        ...
                        PROUNDRECT pRoundRectData)
{
    ...
    ** The input width and height should be the value of the total ellipse
    */
    if (pRoundRectData->height <= 2 || pRoundRectData->width <= 2)
    {
        return DrawRect(pDev, pGraphParam, (PRECTDATA)pRoundRectData);
    }
    if (pGraphParam->pPen == NULL)
