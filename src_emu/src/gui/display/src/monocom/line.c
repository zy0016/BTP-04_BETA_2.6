    ...
    case LINE_HORIZONTAL:
        ...
        if (pPen->style == PS_SOLID)
        {
            DrawSolidHLine(pDev, LineDraw.start_x, LineDraw.end_x, 
                LineDraw.start_y, rop, pPen->fgcolor);
        }
        else
        {
    ...
    case LINE_VERTICAL:
        ...
        if (pPen->style == PS_SOLID)
        {
            DrawSolidVLine(pDev, LineDraw.start_y, LineDraw.end_y, 
                LineDraw.start_x, rop, pPen->fgcolor);
        }
        else
        {
            penstyle = GetPenStyle(pPen->style);
...
//        }
        ...
        */
        FillSolidRect(pDev, &LineRC, pGraphParam->pPen->fgcolor, rop);
    }
...
int MONO_HW_HLine(PDEV pDev, int x1, int x2, int y)
{
    ...
    uint16 dwords;
    uint32 *pDst, *pVal, *pEnd;
    int h;

    // Computes left mask and right mask
    LeftMask = GetLeftMask32(x1 & 0x0F);
    RightMask = GetRightMask32(x2 & 0x0F);
    ...
    for(h = 0; h < 2; h++)
    {
        *pDst = *pDst & ~LeftMask;
        ...
        {
            pEnd = pDst + dwords;
            for(pVal = pDst + 1; pVal < pEnd; pVal++)
                *pVal = 0;
                    ...
                    PRECTDATA pRectData)
{
    ...
    GraphParam.bk_color = pPen->fgcolor;
