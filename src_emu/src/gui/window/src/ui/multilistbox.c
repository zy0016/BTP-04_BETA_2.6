...
#define LARROW_WIDTH	17
#define	LARROW_HEIGHT	16

static LRESULT CALLBACK MultiListBox_Func(HWND hWnd, UINT uMsgCmd, 
...
BOOL MULTILISTBOX_RegisterClass(void)
{
	...
	wc.style            = CS_OWNDC;
    wc.lpfnWndProc      = MultiListBox_Func;
    wc.cbClsExtra       = 0;
                                 ...
                                 WPARAM wParam, LPARAM lParam)
{
	...
	GetClassInfo(NULL, "LISTBOX", &WndClassList);
        ...
        || (uMsgCmd == LB_SETCURSEL))
    {
        ...
        if (nIndexLast != LB_ERR)
            bSelLast = CallWindowProc(MultiListBox_Func, hWnd, LB_GETSEL, nIndexLast, 0);
    }
	...
	case LB_INSERTSTRING:
        if (lResult >= 0)
        {
            ...
            if (pMultiListBoxData->pbSelected != NULL)
            {
                pMultiListBoxData->nMaxCount++;
                pMultiListBoxData->pbSelected = (BYTE*)LocalRealloc(
                    pMultiListBoxData->pbSelected,
                    pMultiListBoxData->nMaxCount * sizeof(BYTE));
                if (uMsgCmd == LB_ADDSTRING)
                {
                    pMultiListBoxData->pbSelected[pMultiListBoxData->nMaxCount - 1]
                        = FALSE;
                }
                else
                {
                    for (i = pMultiListBoxData->nMaxCount - 1; i > (int)lResult; i--)
                    {
                        pMultiListBoxData->pbSelected[i]
                            = pMultiListBoxData->pbSelected[i - 1];
                    }
                    pMultiListBoxData->pbSelected[i] = FALSE;
                }
            }
        }
        break;
    ...
    case LB_DELETESTRING:
        if ((lResult != LB_ERR) && (pMultiListBoxData->pbSelected != NULL))
        {
            for (i = (int)wParam; i < pMultiListBoxData->nMaxCount - 1; i++)
            {
                pMultiListBoxData->pbSelected[i]
                    = pMultiListBoxData->pbSelected[i + 1];
            }
	...
	case WM_KEYDOWN:
        ...
        case VK_UP:
            ...
            bSelected = CallWindowProc(MultiListBox_Func, hWnd, LB_GETSEL,
                nCurIndex, 0);
            if (nIndexLast != LB_ERR)
            {
                CallWindowProc(WndProcList, hWnd, LB_SETIMAGE, 
